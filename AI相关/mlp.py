...
# 具体推导见：https://zhuanlan.zhihu.com/p/621327569
# 训练模型
for epoch in range(epochs):
    ...
    d_output = output_error * sigmoid_derivative(y_hat) # 链式法则，激活函数求导，矩阵点乘, shape of (4, 1)

    hidden_layer_error = d_output @ weights_hidden_output.T # shape of (4, 1) @ (1, 4)
    hidden_layer_delta = hidden_layer_error * sigmoid_derivative(hidden_layer_act) # shape of (4, 4)

    # 更新权重和偏置，为什么这里是加号
    weights_hidden_output += learning_rate * (hidden_layer_act.T @ d_output)
