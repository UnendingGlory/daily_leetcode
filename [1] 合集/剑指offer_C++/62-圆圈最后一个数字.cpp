...
// Solution3: 动态规划。
// 以上两种方法易写，但是在 n和 m过大时会超时
// 接下来通过数学分析的方法，使用动态规划解决
// 输入 n, m，记该 [n, m]问题的解为 f(n)
// 则 [n, m]：环为0, 1, 2, .. n - 1，解为 f(n)
// [n-1, m]：环为0, 1, 2, .. n - 2，解为 f(n-1)

// 对于 [n, m]问题，由于 m可能大于 n，因此首轮删除的数为 (m - 1) % n 
// 从下一个数t = m % n开始，可得环记为 Ring
// t, t + 1, t + 2 ... 0, 1, ... t - 2

// 观察 Ring 环和 [n-1, m]问题，发现位置是一一对应的
// [n-1, m]问题中某数 x，对应 Ring中元素为 (x + t) % n

// 又因为 [n, m]问题的解是由 Ring的解得来的
// 所以如果知道了 [n-1, m]问题的解为 f(n-1)，根据对应关系
// Ring的解为 (f(n - 1) + t) % n，即 [n, m]问题的解 f(n)
