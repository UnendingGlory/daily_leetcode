...
# Solution3: 对于2进行优化。
# 本质上，即对于每个元素 h，寻找左边第一个小于 h的元素的下标 left，和右边第一个小于 h的元素的下标 right。
# area = (right - left - 1) * h

# 如何寻找左边第一个小于 h的元素？该元素对应下标记为 left。
# 可以使用单调递增栈，栈中存的是下标，栈底到栈顶下标对应的元素严格递增。
# 从左到右遍历，当遍历到下标对应的元素尾 num时，将栈中所有对应元素大于等于 num的下标 pop出。
# 从栈中拿到的第一个小于 h的下标即想得到的 left。
# 极端情况下，如果栈为空，那么左侧元素均大于等于 h，可以认为 left = -1。

# 如何寻找右边第一个小于 h的元素？该元素对应下标记为 right。
# 使用单调递增栈，栈底到栈顶下标对应的元素严格递增。
# 从左到右遍历，当遍历到下标对应的元素 num时，将栈中所有对应元素大于等于 num的下标 pop出。
# 从栈中拿到的第一个小于 h的下标即想得到的 right。
# 极端情况下，如果栈为空，那么右侧元素均大于等于 h，可以认为 right = n。
